          ...
          <div key={document.id} className="rounded-lg border border-[#e2e3dc] bg-[#f9faf3] p-3">
            <div className="flex items-center justify-between gap-2">
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold text-[#1a1c18]">{document.title}</p>
                <p className="text-xs text-[#42493f]">{docLabels[document.type]} · {document.updatedAt}</p>
              </div>
              <span className="shrink-0 rounded-full bg-[#e8e9e2] px-2.5 py-1 text-xs font-semibold text-[#42493f]">
                {document.status}
              </span>
            </div>
          </div>
