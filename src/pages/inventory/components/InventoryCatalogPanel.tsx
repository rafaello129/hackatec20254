            ...
            <article key={item.id} className="rounded-lg border border-[#e2e3dc] bg-[#f9faf3] p-3">
              <div className="mb-1 flex items-start justify-between gap-2">
                <div className="flex items-start gap-2">
                  <span className="mt-0.5 inline-flex h-7 w-7 items-center justify-center rounded-md bg-white text-[#4F7302]">
                    {item.kind === "service" ? <Wrench className="h-4 w-4" /> : <ShoppingBag className="h-4 w-4" />}
                  </span>
                  <div>
                    <p className="text-sm font-semibold text-[#1a1c18]">{item.name}</p>
                    <p className="text-xs text-[#42493f]">{categoryLabels[item.category]}</p>
                  </div>
                </div>
                <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${availability.className}`}>
                  {availability.label}
                </span>
              </div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {item.availableForCooperative ? (
                  <span className="inline-flex items-center gap-1 rounded-full bg-[#D6D979] px-2.5 py-1 text-xs font-semibold text-[#3E5902]">
                    <Handshake className="h-3.5 w-3.5" />
                    Apto cooperativo
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1 rounded-full bg-[#e8e9e2] px-2.5 py-1 text-xs font-semibold text-[#42493f]">
                    <Boxes className="h-3.5 w-3.5" />
                    Uso interno
                  </span>
                )}
                <span className="rounded-full bg-white px-2.5 py-1 text-xs font-semibold text-[#42493f]">
                  {useCaseLabels[item.cooperativeUseCase]}
                </span>
                {item.bulkPurchaseEligible ? (
                  <span className="rounded-full bg-white px-2.5 py-1 text-xs font-semibold text-[#42493f]">
                    Bulk min {item.minimumBulkQuantity}
                  </span>
                ) : null}
              </div>
            </article>
